...
A scheduled trigger is a trigger that activates on a set schedule, defined by a
`schedule` parameter object. A scheduled trigger includes the common trigger
attributes along with a required `schedule` parameter:
...
The `schedule` object can take the shape of either a single occurrence schedule
or a recurring schedule. A single occurrence schedule is a schedule which will
only activate one time, a recurring schedule sets a schedule that will repeat on
the schedule.
...
A Single occurrence schedule can be created using just these two parameters or
along with these parameter explicilty mentioning once frequency type, however a
recurring schedule may also include the following parameters:
...
The `frequency` object contains information on the repeating schedule for the
trigger, it contains the following parameters:
...
The scheduled trigger also has access to a data context object which includes
information related to the context of the trigger activation. This data object
can be used to fill the optional input fields of the trigger being activated.
The data context parameters available to a scheduled trigger are as follows:
...
Below are some usage examples of `ScheduledTrigger` objects which can be used in
a .ts file to create a `scheduled` trigger through the Slack CLI, alternatively
this object could be passed into a `client.workflows.triggers.create` method to
achieve the same effect at runtime.
